...
class ScrapingController extends Controller
{
    ...
    public function scrape()
    {
        ...
        for ($page = 1; $page <= $totalPages; $page++) {
            ...
            $oldPrices = $xpath->query("//div[contains(@class, 'promotion-item__discount-component')]//s[contains(@class, 'andes-money-amount--previous')]//span[contains(@class, 'andes-money-amount__fraction')]");    
            ...
            if ($names->length === 0 || $oldPrices->length === 0 || $newPrices->length === 0) {
                die("Erro ao extrair informações da página. Verifique o HTML retornado e os XPaths.");
            }
    
            for ($i = 0; $i < $names->length; $i++) {
                $name = $names->item($i)->nodeValue;
    
                $oldPrice = $oldPrices->length > $i ? $oldPrices->item($i)->nodeValue : '0';
                $newPrice = $newPrices->length > $i ? $newPrices->item($i)->nodeValue : '0';
    
                // Remover caracteres não numéricos e converter para decimal
                $oldPrice = floatval(str_replace(',', '.', preg_replace('/[^\d,]/', '', $oldPrice)));
                $newPrice = floatval(str_replace(',', '.', preg_replace('/[^\d,]/', '', $newPrice)));
